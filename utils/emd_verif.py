...
def emd_verif(anchor, anchor_center, fb, fb_center, stage, method=''):
    flows = None
    u = v = None
    if stage == 0:  # stage 1: Cosine similarity
        ...
        torch_vector = torch.from_numpy(anchor).unsqueeze(0)  # Shape: torch.Size([1, 512])
        anchor = torch_vector.squeeze()
        
        anchor_center = torch.from_numpy(anchor_center)  # Shape: torch.Size([512,16])
        
        
        fb = torch.from_numpy(fb).unsqueeze(0)  # Shape: torch.Size([1, 512])

        
        fb_center = torch.from_numpy(fb_center).unsqueeze(0)  # Shape: torch.Size([1, 512,16])
        

        #fb_center = torch_vector.squeeze()
        ...
        print(type(anchor_center))
        print("fb")
        print(fb.shape)
        print(type(fb))
        print("fb center")
        print(fb_center.shape)
        print(type(fb_center))
