...
from face_models.iresnet import * #add Pedro
...
from data_loader.facedata_loaderRGB import get_face_dataloaderRGB #add Pedro
...
def main():
    ...
    if args.fm == 'arcface':
        ...
        datasets = { 'lfw':['lfw128','lfw128_masked','lfw128_glass', 'lfw128_crop'],
                'lfw_1680':['lfw128_1680'], 'mfr2':['mfr2'], }
    ...
    if args.fm == 'arcface':
        ...
        model = iresnet100(num_features=512).to("cuda:2")
        print(model)
        try:
            model_pth = "pretrained/MaskInvHG_backbone.pth"
            if not os.path.exists(model_pth):
                raise Exception("Model file does not exist!", model_pth)
            model.load_state_dict(torch.load(model_pth, map_location=torch.device("cuda:2")))
            model.train(False)
        except Exception:
            traceback.print_exc()
